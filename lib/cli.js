...
async function start() {
  ...
  for (let [name, suite] of timeMap) {
    ...
    specWeights[name] = { time: suite.duration, weight: 0, failures: suite.errors };
    table.push([
      name,
  ...
  //logs the table with or without colors
  settings.color ? console.log(table.toString()) : console.log(table.toString().replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, ''));
  ...
  errorMap.forEach((error, suiteName) => {
    ...
    error.forEach((errorResult, i) => {
      ...
      console.log(`      ${settings.color ? colors.red(errorResult.message) : errorResult.message}`);
      console.log()
    })
  });
