...
function generateWeightsFile(specWeights, totalDuration, totalWeight) {
  ...
  let oldWeightFile = {};
  try { oldWeightFile = JSON.parse(fs.readFileSync(settings.weightsJSON, 'utf8'));}
  catch (err) { console.log(`Weight file not found in path: ${settings.weightsJSON}`)}

  Object.keys(specWeights).forEach((spec) => {
    if(specWeights[spec].failures == 0){
        oldWeightFile[spec] = {
            weight: Math.ceil((specWeights[spec].time / totalDuration) * totalWeight) + 1,
            time: specWeights[spec].time
        }
    }
  });

  const weightsJson = JSON.stringify(oldWeightFile);
  try {
