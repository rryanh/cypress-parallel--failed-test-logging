...
function createReporterConfigFile(path) {
  const reporterEnabled = ['cypress-parallel--failed-test-logging/json-stream.reporter.js'];
  let reporterName = settings.reporter;
  if (settings.reporter) reporterEnabled.push(reporterName);
  else reporterEnabled.push('cypress-parallel--failed-test-logging/simple-spec.reporter.js');
  const content = {
